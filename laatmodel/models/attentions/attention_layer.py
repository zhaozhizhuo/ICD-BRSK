...
class AttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.linear = nn.Linear(self.size*2, self.size*2, bias=False)
        if self.attention_mode == "hard":
            ...
            self.first_linears = nn.ModuleList([nn.Linear(self.size*2, self.d_a, bias=False) for _ in range(self.n_level)])
    ...
    def _init_weights(self, mean=0.0, std=0.03) -> None:
        ...
        for first_linear in self.first_linears:
            nn.init.normal_(first_linear.weight, mean, std)
