...
class AttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.level_projection_size = level_projection_size          #128

        self.linear = nn.Linear(self.size, self.size, bias=False)   # 1024,1024  --全连接层
        if self.attention_mode == "hard":
            ...
            self.first_linears = nn.ModuleList([nn.Linear(self.size, self.d_a, bias=False) for _ in range(self.n_level)])    #两层线性层（全连接层） 1024*512
            self.second_linears = nn.ModuleList([nn.Linear(self.d_a, self.n_labels[label_lvl], bias=False) for label_lvl in range(self.n_level)])   #512*27   512*8409
            self.third_linears = nn.ModuleList([nn.Linear(self.size +(self.level_projection_size if label_lvl > 0 else 0),
    ...
    def _init_weights(self, mean=0.0, std=0.03) -> None:
        ...
        for first_linear in self.first_linears:
            torch.nn.init.normal(first_linear.weight, mean, std)    #初始化第一层全连接的权重
