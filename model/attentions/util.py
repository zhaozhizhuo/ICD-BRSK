...
def init_attention_layer(model):
    if model.args.joint_mode == "flat":
        ...
        if model.attention_mode is not None:
            model.attention = AttentionLayer(args=model.args, size=model.output_size,
                                             ...
                                             n_labels=model.vocab.all_n_labels(), n_level=model.vocab.n_level())        #n_labels来表示一共有多少个输出，来控制输出的纬度
        linears = []
        projection_linears = []
        for level in range(model.vocab.n_level()):
            level_projection_size = 0 if level == 0 else model.level_projection_size        #当level=0的时候表示是icd的第一层结构预测
    ...
    if model.attention_mode is not None:
        model.r = model.attention.r     #代表icd编码两层的分别的数量
